...
- 为低内存服务器新增前端运行时镜像构建方式：服务器只接收本地已构建 `dist`，不再执行 Node/Vite 构建。
- 部署提交 `2fbcd83` 至公网服务器 `1.94.218.30:18000`：上传本地后端 JAR 和前端 `dist`，使用 runtime Compose 重建后端与前端。
- 远程 Flyway 成功执行 V2 迁移，schema 从版本 1 升级到版本 2；MySQL、后端、前端容器均 healthy。
- 公网冒烟通过：HTTP 首页、注册、兴趣标签保存、推荐文章、管理员成员列表、管理员帖子列表、帖子隐藏和公开列表过滤。
- 公网测试账号 `cbadmin1782264653` 仅用于验收管理员端和推荐闭环；测试文章 ID 为 9，已通过管理员接口隐藏。

## 2026-06-25

- 开始完整黑盒/白盒测试与报告阶段。
- 已复核当前测试报告和测试代码，确认需要补充安全、越权、评论、校验、管理员停用和前端 SSE 解析等用例。
- 计划输出：更新 `docs/test-report.md`，新增正式 Word 测试报告，并把执行证据记录到仓库。
- 已新增后端安全/越权/评论/校验/限流测试，后端测试从 6 个扩展到 12 个并通过。
- 已新增前端 `streamPost` SSE 流式解析测试，前端测试从 1 个扩展到 3 个并通过。
- 完整验证通过：后端 `mvn verify`、前端 `npm test`、前端 `npm run build`、runtime Compose config。
- JaCoCo 后端行覆盖率 77.85%，指令覆盖率 74.11%，方法覆盖率 84.14%，超过 60% 行覆盖率标准。
...
- UI 优化提交 `87cba2d` 已推送至 GitHub `refactor/campus-blog`；公网服务器已使用前端 runtime 镜像重建，`http://1.94.218.30:18000` 返回 HTTP 200，前端、后端、MySQL 容器均 healthy。
