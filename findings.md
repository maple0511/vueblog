# Findings

## 旧仓库

- 最后提交时间为 2020-10-20，约 1673 行 Java/Vue/JS。
- Spring Boot 2.2、Java 8、Vue 2、Shiro、MD5、硬编码数据库密码与 JWT 密钥。
- CORS 允许任意来源，测试仅包含空的上下文加载。
- Dockerfile 暴露端口与实际服务端口不一致，Compose 缺少初始化、健康检查和密钥管理。

## 课程要求

- 本地模板要求需求构思、SRS、用例/时序模型、SDS、体系结构/类图/部署图、数据库设计、黑白盒测试、质量分析、答辩 PPT 和团队/个人贡献统计。
- 数据库设计与测试报告模板为空文件，需要生成有效文档。

## 实施约束

- 周期 3–5 天，团队 4 人以上。
- 产品定位为校园知识博客。
- AI 功能包括摘要、标签、写作助手和当前文章问答。
- 公网验收使用已有 Linux 服务器的 IP + 端口。

## 千问兼容接口

- AI 厂商确定为阿里云百炼千问，模型为 `qwen3.7-plus`。
- 用户提供的 Base URL 已包含 `/compatible-mode/v1`，客户端不得再次重复追加 `/v1`。
- `extra_body={"enable_thinking": true}` 在兼容 HTTP 请求中对应顶层字段 `enable_thinking`。
- 流式响应可能包含 `reasoning_content`；产品只向前端发送最终 `content`，不暴露内部思考过程。
- Spring MVC `SseEmitter` 完成时会触发 ASYNC dispatcher；若安全链只认证初始 REQUEST，会在响应已提交后产生 403 并导致 Nginx 报上游提前关闭。

## 管理员与推荐扩展

- 当前数据模型没有角色、用户状态、兴趣偏好和帖子审核状态，需要新增 Flyway 迁移，避免直接改 `V1` 破坏已部署数据库。
- 为保持原有“发布后可见”的验收闭环，帖子审核状态默认 `APPROVED`；管理员审核作为治理能力，而不是阻塞创作流程。
- 兴趣推荐可以复用现有标签体系：用户偏好保存为标签名，推荐页查询与这些标签匹配的已审核文章；没有偏好时返回最新已审核文章。

## 测试覆盖发现

- 现有测试已覆盖注册/当前用户、未登录保护、文章创建搜索、AI 未配置降级、管理员隐藏、推荐匹配和 OpenAI 兼容 Provider 解析。
- 缺口包括：重复注册、错误登录、管理员停用后登录失败、非管理员访问后台、文章越权修改、评论发布/删除权限、标签长度校验提示、兴趣标签数量校验、AI 流式前端解析。
- 现有 `docs/test-report.md` 偏摘要，缺少完整黑盒用例执行表、白盒路径分析、环境说明、缺陷记录、覆盖率证据和结论；需要扩展并生成 Word 版。

## UI 视觉优化发现

- 现有前端功能完整，但整体视觉偏平铺，首页、列表、编辑器、文章详情、后台之间缺少统一的空间层级和品牌感。
- 当前页面结构已经覆盖主要业务场景，优先用全局 CSS 变量、卡片阴影、渐变背景、悬浮反馈和响应式规则增强表现力，避免改动接口、状态流和模板结构。
- 课程演示更需要“完整产品感”：首页需要有明显 Hero 叙事，文章列表需要卡片化，AI 写作与问答区域需要突出创新功能，管理员端需要体现治理后台属性。
